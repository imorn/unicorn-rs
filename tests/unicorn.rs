...
use std::rc::Rc;
use unicorn_rs::unicorn_const::{uc_error, Arch, HookType, MemType, Mode, Permission, SECOND_SCALE};
use unicorn_rs::{InsnSysX86, RegisterARM, RegisterMIPS, RegisterPPC, RegisterX86};
...
type Unicorn<'a> = unicorn_rs::UnicornHandle<'a>;

#[test]
fn emulate_x86() {
    let x86_code32: Vec<u8> = vec![0x41, 0x4a]; // INC ecx; DEC edx

    let mut unicorn = unicorn_rs::Unicorn::new(Arch::X86, Mode::MODE_32)
...
fn x86_insn_sys_callback() {
    ...
    let mut unicorn = unicorn_rs::Unicorn::new(Arch::X86, Mode::MODE_64)
        .expect("failed to initialize unicorn instance");
    let mut emu = unicorn.borrow();
    assert_eq!(emu.mem_map(0x1000, 0x4000, Permission::ALL), Ok(()));
...
fn emulate_arm() {
    ...
    let mut unicorn = unicorn_rs::Unicorn::new(Arch::ARM, Mode::THUMB)
        .expect("failed to initialize unicorn instance");
    let mut emu = unicorn.borrow();
...
fn emulate_mips() {
    ...
    let mut unicorn = unicorn_rs::Unicorn::new(Arch::MIPS, Mode::MODE_32)
        .expect("failed to initialize unicorn instance");
    let mut emu = unicorn.borrow();
    assert_eq!(emu.mem_map(0x1000, 0x4000, Permission::ALL), Ok(()));
...
fn emulate_ppc() {
    ...
    let mut unicorn = unicorn_rs::Unicorn::new(Arch::PPC, Mode::PPC32)
        .expect("failed to initialize unicorn instance");
    let mut emu = unicorn.borrow();
    assert_eq!(emu.mem_map(0x1000, 0x4000, Permission::ALL), Ok(()));
...
fn x86_context_save_and_restore() {
    for mode in vec![Mode::MODE_32, Mode::MODE_64] {
        ...
        let mut unicorn = unicorn_rs::Unicorn::new(Arch::X86, mode)
            .expect("failed to initialize unicorn instance");
        let mut emu = unicorn.borrow();
        assert_eq!(emu.mem_map(0x1000, 0x4000, Permission::ALL), Ok(()));
        assert_eq!(emu.mem_write(0x1000, &x86_code), Ok(()));
        ...
        let mut unicorn2 = unicorn_rs::Unicorn::new(Arch::X86, mode)
            .expect("failed to initialize unicorn instance");
